...
%macro connectS(ds=,stdvble=,confidx=,subgidx=,vivble=,sampsize=,gpath=,
                imgname=connectS_plot,h=,w=,tit=,refs=,subgfmt=,conffmt=,fs=8);
data ds1;
   set &ds;

   if abs(&stdvble) > 0.25 then stdcat=3;
   else if 0.15 < abs(&stdvble) <= 0.25 then stdcat=2;
...
proc format;
   value catlev 0="ASMD (*ESC*){unicode '2264'x} 0.10" 1="ASMD 0.10 - 0.15" 2="ASMD 0.15 - 0.25" 3="ASMD > 0.25";
run;
